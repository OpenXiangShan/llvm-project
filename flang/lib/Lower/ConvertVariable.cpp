...
namespace {
  ...
  static bool isComplexDerivedType(const Fortran::semantics::Symbol &sym) {
    ...
    for (const auto &compName : typeDetails.componentNames()) {
      ...
      const auto *objDetails = compSym.detailsIf<Fortran::semantics::ObjectEntityDetails>();
      const auto *procDetails = compSym.detailsIf<Fortran::semantics::ProcEntityDetails>();
  ...
  static void
  genDerivedTypeComponentInit(Fortran::lower::AbstractConverter &converter,
                              mlir::Location loc,
    ...
    for (const auto &compName : typeDetails.componentNames()) {
      ...
      auto fieldIdx = builder.create<fir::FieldIndexOp>(
          ...
          mlir::ValueRange{});
      auto compAddr = builder.create<fir::CoordinateOp>(
          loc, builder.getRefType(compFirTy), baseAddr,
        ...
        if (objDetails->init()) {
          ...
          if (Fortran::semantics::IsPointer(compSym)) {
            ...
            if (mlir::isa<fir::SequenceType>(compFirTy)) {
              std::string globalName = fir::NameUniquer::doGenerated(
                  (converter.mangleName(derivedSpec) + fir::kNameSeparator + name +
                   fir::kNameSeparator + "arr_init").str());
              fir::GlobalOp global = builder.getNamedGlobal(globalName);
              if (!global) {
                ...
                global = Fortran::lower::tryCreatingDenseGlobal(
                    ...
                    builder.createInternalLinkage(), /*isConst=*/true,
                    initExpr, dataAttr);
              }
              if (global) {
                auto srcAddr = builder.create<fir::AddrOfOp>(loc, global.resultType(),
                                                           global.getSymbol());
                builder.create<fir::CopyOp>(loc, srcAddr, compAddr, /*noOverlap=*/true);
              } else {
                Fortran::lower::StatementContext inlineCtx;
                Fortran::lower::SymMap emptyMap; 
                hlfir::Entity lhs{compAddr};
                hlfir::Entity rhs = Fortran::lower::convertExprToHLFIR(
                    loc, converter, objDetails->init().value(), emptyMap, inlineCtx);
          ...
          std::string globalName = fir::NameUniquer::doGenerated(
              (converter.mangleName(derivedSpec) + fir::kNameSeparator + name +
               fir::kNameSeparator + "null_box").str());
          fir::GlobalOp global = builder.getNamedGlobal(globalName);
          if (!global) {
            global = builder.createGlobal(loc, compFirTy, globalName,
                                         builder.createInternalLinkage(),
                                         mlir::Attribute{}, true);
            createGlobalInitialization(builder, global, [&](fir::FirOpBuilder &b) {
              mlir::Value nullBox = fir::factory::createUnallocatedBox(b, loc, compFirTy, mlir::ValueRange{});
          ...
          if (mlir::isa<fir::SequenceType>(compFirTy) && isComplexDerivedType(compSym)) {
            // Complex derived type arrays.
            mlir::Value box = builder.createBox(loc, fir::ExtendedValue(compAddr));
            fir::runtime::genDerivedTypeInitialize(builder, loc, box);
          } else if (isComplexDerivedType(compSym)) {
            // Complex derived type scalars.
            const Fortran::semantics::DeclTypeSpec *compDeclTy = compSym.GetType();
        ...
        if (procDetails->init().has_value()) {
          ...
          if (!global) {
            ...
            createGlobalInitialization(builder, global, [&](fir::FirOpBuilder &b) {
              ...
              if (targetSym) {
                procBox = Fortran::lower::convertProcedureDesignatorInitialTarget(
...
void Fortran::lower::defaultInitializeAtRuntime(
  ...
  if (Fortran::semantics::IsOptional(sym)) {
    ...
    const auto *details =
        sym.detailsIf<Fortran::semantics::ObjectEntityDetails>();
    bool useFastInit = details && !Fortran::semantics::IsPolymorphic(sym) &&
                       declTy->category() == 
    ...
    if (useFastInit) { 
      if (enablePreciseInit) {
        ...
        genDerivedTypeComponentInit(converter, loc, derivedSpec, baseAddr, recTy);
      } else {
        std::string globalName = fir::NameUniquer::doGenerated(
